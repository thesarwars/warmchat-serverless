...
import { mockElasticSendEmail } from "./mockSendApi.ts";
...
async function sendOwnerEmail(
  ...
  // Route through the mock-aware wrapper so the owner alert lands in the
  // /admin/debug feed (and skips the wire) when mock mode is on. When off it
  // passes straight to ElasticEmail. The wrapper handles the missing-key path,
  // so no early return is needed here - and it still logs in dev where the
  // shared sender key may be unset.
  try {
    await mockElasticSendEmail(
