...
async function updateLead(env: Env, userId: number, leadId: number, body: Record<string, unknown>): Promise<Response> {
  ...
  // Mark any tracked field the agent edited by hand (the governed "smart filter"
  // fields + Timeline / Pre-Approved / Notes) as source='manual' so the
  // background AI extraction won't overwrite it unless new high-confidence
  // conversation evidence shows the lead's intent changed.
  const manualFields = Object.keys(body).filter(isManualTrackedField);
