...
import {
  APPT_PROPOSED, APPT_CONFIRMED,
  serializeAppointment, appointmentToUnifiedDict, type AppointmentRow,
} from "../../../../../../_shared/appointments.ts";

/** POST /api/inbox/contacts/:leadId/appointments/:appointmentId/confirm */
export const onRequestPost: PagesFunction<Env> = async (context) => {
  ...
  if (appt.status !== APPT_PROPOSED) return error("Only proposed appointments can be confirmed", 400);

  const now = nowIso();
  await execute(
    env.D1DB,
    `UPDATE lead_appointment SET status = ?, confirmed_at = ?, updated_at = ? WHERE id = ?`,
    APPT_CONFIRMED, now, now, apptId,
  );
