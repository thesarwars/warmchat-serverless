...
interface TaskRow {
  ...
  lead_name: string | null; created_at: string;
}
...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const rows = await queryAll<TaskRow>(
    env.D1DB,
    `SELECT t.id, t.title, t.description, t.type, t.priority, t.due_at, t.status, t.source,
            t.lead_id, t.deal_id, t.user_id, t.created_at,
            l.name AS lead_name
       FROM task t
