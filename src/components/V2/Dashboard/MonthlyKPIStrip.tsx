...
const KPICard: React.FC<KPICardConfig> = ({ label, value, delta, current, goalValue, goalLabel, icon }) => {
  const hasGoal = (goalValue ?? 0) > 0;
  const progress = hasGoal ? Math.min(100, Math.round((current / (goalValue as number)) * 100)) : 0;
  return (
    <div className="rounded-[18px] border border-[#EAEAEA] bg-white p-4 shadow-[0_1px_2px_rgba(50,35,20,0.03)]">
      <div className="flex items-center gap-3.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          {delta ? <div className="mt-1 text-[11.5px] font-semibold text-[#1f7a52]">{delta}</div> : null}
        </div>
      </div>
      {hasGoal ? (
        <div className="mt-3">
  ...
  return (
    <div>
      <div className="mb-2.5 flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          <div className="min-w-24 text-center text-[13px] font-semibold text-[#211a14]">{monthLabel}</div>
          {onEditGoals ? (
            <button
              type="button"
              onClick={onEditGoals}
