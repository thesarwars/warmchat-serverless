...
export async function checkQuietHours(
  ...
  const timezone = (leadTimezone || org?.timezone || "").trim();
  if (!timezone) return null;

  const startHour = Number.isFinite(org?.quiet_hours_start) ? Number(org!.quiet_hours_start) : 8;
  const endHour = Number.isFinite(org?.quiet_hours_end) ? Number(org!.quiet_hours_end) : 21;

  const now = new Date();
  const hour = hourInZone(now, timezone);
  // Window is [startHour, endHour). Anything outside that is blocked.
  const blocked = hour < startHour || hour >= endHour;

  return {
    blocked,
    hour,
    timezone,
    until: nextOpening(now, timezone, startHour),
  };
}
